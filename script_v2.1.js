document.addEventListener("DOMContentLoaded", () => {
    ...
    function guardarDatos() {
        ...
        localStorage.setItem('juegoActivo', !juegoDiv.classList.contains('hidden') ? "true" : "false");
    }

    function mostrarJuego(cargarPersonas) {
        juegoDiv.classList.remove("hidden");
        inicioDiv.classList.add("hidden");
        cargarPersonasDiv.classList.add("hidden");

        if (cargarPersonas && personas.length > 0) {
